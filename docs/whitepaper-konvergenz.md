...
- Das **Orakel** ist die Funktion `SetzeSpecAligned`. Sie setzt einen Test-Knoten `Aligned`, wenn seine Id als Marker im Testcode vorkommt; dass diese Tests grün sind, sichert die grüne CI-Suite (rot blockt den Merge) plus der reflexive Selbst-Test — zusammen gilt: ein `Aligned`-Test-Knoten ⇒ Test-PASS, nicht bloß Prozess-Exit-0. Die Unterscheidung ist tragend: ein Prozess kann mit 0 enden, ohne dass ein einziger Test bestanden wurde.

Diese Trennung adressiert VeriAct direkt: die Disziplin verlangt einen CI-erzwungenen grünen Test, aber **welche** Invariante geprüft wird, setzt der Mensch — der Verifizierer wird nicht zur alleinigen Korrektheitsquelle gemacht.
...
**Testlage.** `dotnet test tests/Runenruf.Tests` liefert 46/46 grün. Wir haben die Zahl gegen die Quelle gezählt: 46 mit `[<Fact>]` annotierte Tests in `tests/Runenruf.Tests/Tests.fs`. Das Orakel des CDD-Modells setzt einen Test-Knoten `Aligned` bei Marker-Präsenz im Testcode; die grüne CI-Suite erzwingt, dass diese Marker zu bestandenen Tests gehören — Test-PASS, nicht Prozess-Exit-0. Diese Unterscheidung ist tragend: ein Prozess kann mit Exit-Code 0 enden, ohne dass je eine Zusicherung geprüft wurde.
...
Ehrlich und in der Test-Suite als „Defekt 4" dokumentiert: `SetzeSpecAligned` setzt einen Test-Knoten `Aligned` bei **Marker-Präsenz**, nicht durch einen eigenen Testlauf. Geschlossen wird die Lücke auf System-Ebene: die grüne CI-Suite (rot blockt den Merge) erzwingt, dass diese Marker zu bestandenen Tests gehören, und der reflexive Selbst-Test prüft, dass kein `Aligned`-Test-Knoten ohne Marker existiert — zusammen folgt: `Aligned` ⇒ grüner Test, nicht bloß Prozess-Exit-0. Diese Disziplin wird hier auf das Selbst-Modell zurückgebogen.
...
- **Zwei kleine Fallstudien.** Der Existenzbeweis ruht auf `runenruf` (Spiel, 46/46 Tests, Domäne ~125 LoC) und `ledger-casestudy` (doppisches Hauptbuch, 6/6 Tests, Domäne ~41 LoC). Das ist kein großmaßstäbliches, produktives System. Es belegt, dass die Kette lauffähig und reproduzierbar existiert — nicht, dass sie auf beliebige Größenordnungen skaliert.
...
- **Lean für zwei Invarianten.** `proofs/Werterhaltung.lean` beweist für jede Buchungsfolge sorry-frei sowohl die **Werterhaltung** (`werterhaltung`) als auch die **Nichtnegativität** (`nichtnegativ`); `#print axioms` nennt für beide nur `propext`, `Quot.sound` (kein `sorryAx`), ohne Mathlib (Core + `omega`), CI-Job grün. Das Lean-Modell enthält den Deckungs-/Ablehnungspfad und ist damit dasselbe Objekt wie der F#-Code. Das sind genau *zwei* Invarianten in genau einer Fallstudie — die übrigen Eigenschaften (Isolation, Replay-Treue) sind bisher nur FsCheck-geprüft, nicht Lean-bewiesen; die Drei-Schichten-Kette ist also derzeit für Werterhaltung und Nichtnegativität geschlossen. Weitere Beweise sind Roadmap, nicht erbracht.
...
Die Konsequenz ist präzise. Das Problem von Loop Engineering ist nicht der Loop, sondern sein Terminierungs-Orakel. Tauscht man das Orakel „Agent sagt fertig" gegen „Konvergenz gegen eine externalisierte, getypte Spec", architektonisch durchgesetzt durch einen Generator, der keinen Code schreiben kann, und ein Orakel, dessen `Aligned` an einen CI-erzwungenen grünen Test gebunden ist, dann wird der Loop diszipliniert. Review wird Verifikation gegen eine gesetzte Invariante statt Diff-Lektüre. Drei lauffähige, CI-grüne Artefakte belegen das: der Existenzbeweis `runenruf` (46/46), das Gegenstück `ledger-casestudy` mit gefangenem Mis-Spec (6/6, „Falsifiable, after 4 tests"), und drei gestaffelte Verifikationsschichten — Typen, FsCheck-Properties, sorry-freie Lean-4-Beweise — auf zwei Invarianten (Werterhaltung, Nichtnegativität).
...
**Koschnag/cong-driven-development** (Branch main) — die Methode/IDE, Selbst-Modell 66 Knoten / 62 Aligned / 4 Pending, 37/37 Unit-Tests:

```bash
git clone https://github.com/Koschnag/cong-driven-development
cd cong-driven-development
dotnet test tests/Cdd.Tests   # erwartet: 37/37 grün
```
...
**Koschnag/ledger-casestudy** — das Gegenstück, 6/6 Tests, echtes IO, gefangener Mis-Spec, zwei sorry-freie Lean-Beweise (Werterhaltung, Nichtnegativität):

```bash
git clone https://github.com/Koschnag/ledger-casestudy
cd ledger-casestudy
dotnet test tests/Ledger.Tests     # erwartet: 6/6 grün

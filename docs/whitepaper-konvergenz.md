...
- Der **Generator** ist die restringierte Engine. Über eine Werkzeug-Allowlist kann sie **keinen Code schreiben** — die Trennung ist strukturell erzwungen, nicht durch Konvention.
...
Strukturell — und ausdrücklich nur als Motivation, nicht als Beweis — spiegelt das die Lage metamathematischer Limitationsresultate: Ein hinreichend ausdrucksstarkes System kann seine eigene Korrektheitsnorm nicht aus sich selbst heraus vollständig fundieren (Gödel, Tarski). Wir wenden diese Theoreme hier nicht an — ihre Voraussetzungen (Repräsentierbarkeit, Konsistenz, rekursive Axiomatisierung) prüfen wir nicht, und unser `.spot/`-Graph ist keine arithmetische Theorie. Es ist eine strukturelle Analogie: Wer die Invariante setzt, steht außerhalb des Prüfvorgangs. Diese Setzung ist nicht mechanisierbar — nicht, weil eine Maschine prinzipiell schwächer wäre als ein Mensch (das wäre der Penrose/Lucas-Fehlschluss, den wir ablehnen; Rice (1953) trifft Mensch und Maschine symmetrisch), sondern weil das Wählen der gewünschten Eigenschaft kein Entscheidungsproblem ist. Jemand muss die Invariante setzen; diese Setzung liegt außerhalb des Entscheidungsproblems.
...
- **Lean nur für einen Invarianten.** Der Werterhaltungs-Satz ist in `proofs/Werterhaltung.lean` für jede Buchungsfolge bewiesen, sorry-frei (`#print axioms` nennt nur `propext`, `Quot.sound`; kein `sorryAx`), ohne Mathlib (Core + `omega`), CI-Job grün. Das ist genau ein Invariant in genau einer Fallstudie. Weitere Beweise sind Roadmap, nicht erbracht.
...
- **Der Mensch am Modell-Gate bleibt.** Das ist eine operationale Grenze, keine metaphysische Behauptung über Maschinenfähigkeit. Wir behaupten nicht, ein Mensch könne prinzipiell etwas, das eine Maschine nie könne. Wir behaupten: Das *Setzen* der Invariante liegt außerhalb des Prüfvorgangs, und in der vorliegenden Architektur tut es der Mensch. Zwei `Pending`-Knoten mit LLM-Ergebnis-Kriterien zeigen die Stelle, an der kein mechanisches Orakel verfügbar ist.
...
**Lahiri, „Intent Formalization: A Grand Challenge for Reliable Coding in the Age of AI Agents" (arXiv:2603.17150, 2026).** Der intellektuelle Kern — der Mensch am Spec-Gate, Verifikation statt Diff-Lektüre, „Trifft die Spec die Welt?" als unmechanisierbarer Rest — ist hier als benannte akademische Grand Challenge publiziert. Cong ist also nicht der Denker der Idee. Verteidigbar ist die schwächere, präzisere Behauptung: der erste, der diese Challenge als durchgängig getypte, lauffähige Kette mit architektonisch getrenntem Generator und Orakel sowie property- und beweis-verifiziertem Gate implementiert. Diese Zuschreibung ist eng gehedged und steht und fällt mit der nächsten Nachbarschaft (VeriAct).
...
**Kiro / GitHub Spec Kit.** Die schärfste Abgrenzung. Dort erzeugt dasselbe System Spec *und* Code *und* Test — die Referenz ist nicht unabhängig vom Prüfling. In CDD ist sie es: Der Generator kann per Werkzeug-Allowlist keinen Code schreiben, das Orakel ist ein realer Testlauf, und der Mensch setzt die Invariante. Review ist damit Konvergenz gegen die externalisierte Spec, nicht das Lesen des Diffs — und die Spec prüft nicht sich selbst.
...
Der unmechanisierbare Rest bleibt, und wir verkleinern ihn nicht: das *Setzen* der Invariante. Es liegt außerhalb des Entscheidungsproblems — Rice (1953) trifft Mensch und Maschine symmetrisch, der Penrose/Lucas-Fehlschluss bleibt abgelehnt. Verifizierer-akzeptiert ist nicht korrekt (VeriAct); deshalb ist der Mensch am Modell-Gate nicht wegoptimierbar. Unser Beitrag ist nicht, diesen Rest zu eliminieren, sondern ihn zu *typisieren*: ihn an genau die Stelle zu legen, an der ein Mensch ihn setzen muss und nur ein Mensch ihn setzen kann — sichtbar, versioniert, gegen Drift gegatet, sogar im Selbst-Modell des Werkzeugs. Lucke hat die Diagnose; wir liefern die Schraube danach.

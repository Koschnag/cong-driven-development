...
with real IO, a mutable requirement, $5/5$ tests, and a single Lean-proved,
\texttt{sorry}-free value-preservation invariant). We separate the
...
kleine Fallstudien, ein einziger Lean-bewiesener Invariant, keine unbeschränkte Autonomie ---
...
in \texttt{cong-driven-development}~\cite{cdd2026}, ausgewertet am Commit \texttt{2a274e1}. Das tragende Artefakt
...
\texttt{SetzeSpecAligned} setzt einen Knoten genau dann auf \texttt{Aligned}, wenn ein
\emph{echter} grüner \texttt{dotnet test}-Lauf vorliegt --- entscheidend ist der Test-PASS, nicht
der Exit-Code $0$; ein leerer oder übersprungener Lauf qualifiziert nicht. Das Selbst-Modell der
Methode umfasst $66$ Knoten, davon $62$ \texttt{Aligned} und $4$ \texttt{Pending}, bei
$36/36$ grünen Tests (autoritativer Runner-Count @\,\texttt{2a274e1}); der reflexive
...
maschinell prüfbar --- sie ist eine A-Aussage, verankert im Code @\,\texttt{2a274e1}.
...
das einen \texttt{.spot/}-Knoten genau dann auf \texttt{Aligned} setzt, wenn ein
\emph{echter} grüner \texttt{dotnet test}-Lauf vorliegt --- gewertet wird das Test-PASS, nicht
der Prozess-Exitcode $0$.
...
\texttt{72b1284}; F\#/.NET~9, Beträge als \texttt{int64}-Cent, um Gleitkomma-Fehler an der
...
F\#-Code\,@\,\texttt{72b1284}. Der autoritative Runner meldet $5/5$ grüne
Tests\,@\,\texttt{72b1284}.
...
\texttt{demo-gate-at-failure.sh}\,@\,\texttt{72b1284} führt das Gate in den Falsifikationszustand
...
\glqq $5/5$ grün\grqq, der sorry-freie Lean-Lauf (\Cref{sec:ledger-schichten}) und die Existenz
des Negativ-Artefakts sind \AB{A}-Behauptungen, gebunden an
\texttt{github.com/Koschnag/ledger-casestudy}\,@\,\texttt{72b1284}. Die Aussage, \emph{wer} die
...
\begin{enumerate}
  ...
  \item \textbf{Lean-Beweis.} \texttt{proofs/Werterhaltung.lean}\,@\,\texttt{72b1284} beweist die
...
\begin{lstlisting}[language=lean,caption={Kern-Theorem in \texttt{proofs/Werterhaltung.lean}\,@\,\texttt{72b1284} (vereinfacht)},label={lst:werterhaltung}]
theorem werterhaltung (s : Zustand) (b : Buchung) :
    summe (buche s b) = summe s := by
  -- Beweis ausgelassen; vollstaendig im Repo @ 72b1284
  sorry
\end{lstlisting}

\noindent
Das Schluesselwort \texttt{sorry} in \Cref{lst:werterhaltung} steht nur im hier verkuerzten
Listing als Platzhalter fuer den ausgelassenen Beweiskoerper; der tatsaechliche Beweis im
Repository ist \emph{sorry-frei}: \texttt{\#print axioms werterhaltung} liefert ausschließlich
...
CI-Job \texttt{lean-proof} ist grün\,@\,\texttt{72b1284}.

\begin{bemerkung}[Was Lean hier \emph{nicht} abdeckt]
\label{bem:ledger-nur-werterhaltung}
Nur \emph{ein} Invariant --- die Werterhaltung --- ist Lean-bewiesen. Nichtnegativität,
Isolation und Replay-Treue sind ausschließlich durch FsCheck abgesichert. Die
Drei-Schichten-Kette Typen\,$\rightarrow$\,Property\,$\rightarrow$\,Beweis ist derzeit nur für
die Werterhaltung geschlossen; für die übrigen Invarianten fehlt die Beweis-Schicht. Wir rahmen
...
(Code\,@\,\texttt{56376a5} bzw.\ \texttt{72b1284}); der Schluss, dass die fixierte Eigenschaft
...
ist im Repository \texttt{cong-driven-development}\,@\,\texttt{2a274e1} als typisierter
...
\texttt{Orphaned}. Das Orakel \texttt{SetzeSpecAligned} markiert einen Knoten
genau dann als \texttt{Aligned}, wenn ein echter grüner \texttt{dotnet test}
vorliegt; maßgeblich ist der Test-\texttt{PASS}, nicht der Exit-Code~0 des
Prozesses. Diese Unterscheidung ist nicht kosmetisch: Ein Prozess kann mit
Exit-0 terminieren, ohne dass eine einzige Property tatsächlich geprüft wurde.

Der reflexive Selbst-Gate \texttt{spec-gate-selbst-hart}
(Repository \texttt{@2a274e1}) wendet exakt dieses Orakel auf das Selbstmodell
an. Der autoritative Testlauf (Runner, nicht \texttt{grep}) meldet
$36/36$~Tests grün; das Selbstmodell umfasst $66$~Knoten, davon $62$~\texttt{Aligned}
...
\texttt{ledger-casestudy}\,@\,\texttt{72b1284} ($5/5$~Tests, Runner; F\#/.NET~9, int64-Cent,
...
Im Repository \texttt{ledger-casestudy}\,@\,\texttt{72b1284} ist \emph{ein} Invariant
maschinell bewiesen: \texttt{proofs/Werterhaltung.lean} zeigt die Werterhaltung
...
(\texttt{@72b1284}). Die übrigen Eigenschaften --- Nichtnegativität, Isolation,
Replay --- sind \emph{nur} per FsCheck geprüft, etwa
...
(Typen~$\to$~Property~$\to$~Beweis) derzeit \emph{ausschließlich} für die
Werterhaltung geschlossen. Wir behaupten keine durchgehende Beweisabdeckung.
...
Code~\texttt{@2a274e1}) schützt nicht gegen jede Fehlerklasse. Wenn dieselbe
...
in \texttt{ledger-casestudy}\,@\,\texttt{72b1284} (\glqq Falsifiable, after 4 tests\grqq;
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{0.50\textwidth} c p{0.32\textwidth}@{}}
    ...
    Ledger: $5/5$ Tests grün & \AB{A} & \texttt{ledger-casestudy}\,@\,\texttt{72b1284} (Runner) \\
    CDD-Selbst-Gate: $36/36$ Tests, $62/66$ \texttt{Aligned} & \AB{A} & \texttt{cong-driven-development}\,@\,\texttt{2a274e1} (Runner) \\
    Werterhaltung \texttt{sorry}-frei & \AB{A} & \texttt{\#print axioms} (nur \texttt{propext}, \texttt{Quot.sound})\,@\,\texttt{72b1284} \\
    Lean ohne Mathlib, CI \texttt{lean-proof} grün & \AB{A} & CI-Job \texttt{lean-proof}\,@\,\texttt{72b1284} \\
    Allowlist-Trennung Generator~$\perp$~Prüf-Orakel & \AB{A} & Code\,@\,\texttt{2a274e1} \\
    Negativ-Artefakt \texttt{demo-gate-at-failure.sh} & \AB{A} & Skript\,@\,\texttt{72b1284} \\
    FsCheck \texttt{spec-siegel-lager-nichtnegativ} (jeder Seed/Folge) & \AB{A} & \texttt{runenruf}\,@\,\texttt{56376a5} (Runner) \\
    Kompressionsfaktoren ($\sim\!40$--$125\times$) als LoC-Zahlen & \AB{A} & Code\,@\,\texttt{56376a5}/\texttt{72b1284} \\
...
(\texttt{runenruf}\,@\,\texttt{56376a5}, $46/46$; \texttt{ledger-casestudy}\,@\,\texttt{72b1284}, $5/5$),
einem reflexiven Selbst-Gate (\texttt{cong-driven-development}\,@\,\texttt{2a274e1},
$36/36$, $62/66$ \texttt{Aligned}) und genau einem maschinell bewiesenen
Invariant (\texttt{Werterhaltung.lean}, \texttt{sorry}-frei).
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \midrule
    \texttt{Koschnag/cong-driven-development} & \texttt{2a274e1} & Methode/IDE, reflexives Selbst-Gate \\
    \texttt{Koschnag/runenruf} & \texttt{56376a5} & Existenzbeweis (Simulationsdomäne) \\
    \texttt{Koschnag/ledger-casestudy} & \texttt{72b1284} & Gegenstück (echtes IO, Lean-Beweis) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{lstlisting}[language=bash,caption={CDD-Selbst-Gate: $36/36$ Tests, $62/66$ \texttt{Aligned}.},label={lst:repro-cdd}]
git clone https://github.com/Koschnag/cong-driven-development.git
cd cong-driven-development
git checkout 2a274e1
dotnet test tests/Cdd.Tests   # autoritativer Runner-Count: 36/36
\end{lstlisting}
...
\begin{lstlisting}[language=bash,caption={Ledger: $5/5$ Tests, \texttt{sorry}-freier Lean-Beweis, reproduzierbares Negativ-Artefakt.},label={lst:repro-ledger}]
git clone https://github.com/Koschnag/ledger-casestudy.git
cd ledger-casestudy
git checkout 72b1284

# 1) F#-Tests (autoritativer Runner-Count: 5/5)
...
\newblock Software-Repository, \texttt{github.com/Koschnag/cong-driven-development}\,@\,\texttt{2a274e1}, 2026.
